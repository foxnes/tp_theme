...
$("pre code").each(function(){
    this.innerHTML = this.innerHTML.replace(/(("|'|`)[^\2]*?\2)/ig,"<span class='hl-str'>$1</span>").replace(/(~\/| -{1,2}[a-z]{1,}[ \n\r]|!|\+|\*|%|\||&amp;|==|=&gt;)/ig,"<span class='hl-fh'>$1</span>").replace(/([^a-z0-9]|^)([\d]+)([^a-z0-9]|$)/ig,"$1<span class='hl-num'>$2</span>$3").replace(/([^a-z]|^)(RegExp|return|if|fi|then|else|break|continue|for|endif|in|not|die|exit|switch|case|throw|void|typeof|endwhile|elseif|import|array|while|foreach|important|new|and|or|xor|with|desc|limit|order|try|as|catch)([^a-z]|$)/ig, "$1<span class='hl-kw'>$2</span>$3").replace(/([^:]|^)((#|\/\/).*?)(\n|$)/g,"$1<span class='hl-note'>$2</span>$4").replace(/(\/\*[\s\S]*?\*\/)/g,"<span class='hl-note'>$1</span>").replace(/([^a-z\.]|^)(echo|alert|print|print_r|printf|include|default|final|eval|exec|shell|list|function|static|const|var|unset|unlink|undefined|null|true|false|empty|del|is|def|this|margin|padding|color|background|display|float|font|line|letter|word|cursor|border|vertical|width|height|position|clear|screen|max|min|rgba|rgb|box|inherit|left|right|top|bottom|wrap|wget|ls|chmod|grep|cat|test|match|yum|apt|mv|cp|rm|sudo|sh|source|bash|find|more|touch|awk|curl|tee|tree|read|vim|diff|unzip|tar|ftp|cd|mount|stat|rmdir|git|dd|df|du|restart|stop|start|sfdisk|fdisk|kill|top|free|who|clear|reset|alias|gzip|rpm|pacman|dump|bzip2|dpkg|count)([^a-z])/ig,"$1<span class='hl-kf'>$2</span>$3");
});
